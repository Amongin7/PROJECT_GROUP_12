<?php
session_start();
include('assets/inc/config.php');

if (isset($_POST['add_case_xbt'])) {
    $xbt_compt_name     = $_POST['xbt_compt_name'];
    $xbt_sd_ref         = $_POST['xbt_sd_ref'];
    $xbt_compt_type     = $_POST['xbt_compt_type'];
    $xbt_compt_addr     = $_POST['xbt_compt_addr'];
    $xbt_victim         = $_POST['xbt_victim'];
    $xbt_number         = $_POST['xbt_number'];
    $xbt_desc           = $_POST['xbt_desc'];
    $xbt_offence_details= $_POST['xbt_offence_details'];
    $xbt_date           = date("Y-m-d H:i:s");

    $query = "INSERT INTO his_exhibit 
        (xbt_compt_name, xbt_victim, xbt_sd_ref, xbt_number, xbt_compt_addr, xbt_compt_type, xbt_date, xbt_offence_details, xbt_desc) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param(
        'sssssssss',
        $xbt_compt_name,
        $xbt_victim,
        $xbt_sd_ref,
        $xbt_number,
        $xbt_compt_addr,
        $xbt_compt_type,
        $xbt_date,
        $xbt_offence_details,
        $xbt_desc
    );

    if ($stmt->execute()) {
        $_SESSION['xbt_success'] = true;
        header("Location: " . $_SERVER['PHP_SELF'] . "?sd_ref=" . $xbt_sd_ref);
        exit();
    } else {
        $err = "Please Try Again Or Try Later";
    }
}
?>
<!DOCTYPE html>
<html lang="en">
<?php include('assets/inc/head.php'); ?>
<body>
<div id="wrapper">
    <?php include("assets/inc/nav.php"); ?>
    <?php include("assets/inc/sidebar.php"); ?>

    <?php
    $sd_ref = $_GET['sd_ref'] ?? '';
    $ret = "SELECT * FROM his_cases WHERE sd_ref=?";
    $stmt = $mysqli->prepare($ret);
    $stmt->bind_param('s', $sd_ref);
    $stmt->execute();
    $res = $stmt->get_result();
    while ($row = $res->fetch_object()) {
    ?>

    <div class="content-page">
        <div class="content">
            <div class="container-fluid">

                <div class="page-title-box">
                    <h4 class="page-title">Add Case Exhibits</h4>
                </div>

                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <form method="post">
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            <label>Complainant's Name</label>
                                            <input type="text" name="xbt_compt_name"
                                                   value="<?php echo htmlspecialchars($row->compt_fname . ' ' . $row->compt_lname); ?>"
                                                   class="form-control" readonly required>
                                        </div>
                                        <div class="form-group col-md-6">
                                            <label>Victim(s)</label>
                                            <input type="text" name="xbt_victim"
                                                   value="<?php echo htmlspecialchars($row->victim); ?>"
                                                   class="form-control" readonly required>
                                        </div>
                                    </div>

                                    <div class="form-row">
                                        <div class="form-group col-md-4">
                                            <label>SD Reference</label>
                                            <input type="text" name="xbt_sd_ref"
                                                   value="<?php echo htmlspecialchars($row->sd_ref); ?>"
                                                   class="form-control" readonly required>
                                        </div>
                                        <div class="form-group col-md-4">
                                            <label>Complainant's Address</label>
                                            <input type="text" name="xbt_compt_addr"
                                                   value="<?php echo htmlspecialchars($row->compt_addr); ?>"
                                                   class="form-control" readonly required>
                                        </div>
                                        <div class="form-group col-md-4">
                                            <label>Case Type</label>
                                            <input type="text" name="xbt_compt_type"
                                                   value="<?php echo htmlspecialchars($row->compt_type); ?>"
                                                   class="form-control" readonly required>
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <label>Details of Offence</label>
                                        <input type="text" name="xbt_offence_details"
                                               value="<?php echo htmlspecialchars($row->offence_details); ?>"
                                               class="form-control" required>
                                    </div>

                                    <div class="form-row" style="display: none;">
                                        <div class="form-group col-md-2">
                                            <?php
                                            $pres_no = substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 1, 5);
                                            ?>
                                            <label>Exhibit Number</label>
                                            <input type="text" name="xbt_number"
                                                   value="<?php echo $pres_no; ?>"
                                                   class="form-control">
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <label>Exhibits</label>
                                        <textarea name="xbt_desc" class="form-control" id="editor" required></textarea>
                                    </div>

                                    <button type="submit" name="add_case_xbt" class="btn btn-primary">
                                        Add Case Exhibits
                                    </button>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
        </div>
        <?php } ?>
        <?php include('assets/inc/footer.php'); ?>
    </div>
</div>

<!-- CKEditor -->
<script src="//cdn.ckeditor.com/4.6.2/basic/ckeditor.js"></script>
<script>CKEDITOR.replace('editor');</script>

<!-- Core Scripts -->
<script src="assets/js/vendor.min.js"></script>
<script src="assets/js/app.min.js"></script>
<script src="assets/libs/ladda/spin.js"></script>
<script src="assets/libs/ladda/ladda.js"></script>
<script src="assets/js/pages/loading-btn.init.js"></script>

<!-- Native JS Alert -->
<script>
<?php if (isset($_SESSION['xbt_success']) && $_SESSION['xbt_success']): ?>
    alert('Case Exhibits Added Successfully');
    <?php unset($_SESSION['xbt_success']); ?>
<?php endif; ?>
</script>

</body>
</html>
